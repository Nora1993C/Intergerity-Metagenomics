...
HumanGenome=/project/Ref_database/human_g1k_v37_BWT
...
for n in $(ls $Data/1_Raw_reads/*_combined_R1.fastq);do $BWA/bwa mem -t 90 $HumanGenome/human_g1k_v37 $n ${n%_combined_R1.fastq}_combined_R2.fastq > ${n%_combined_R1.fastq.gz}.humRef.aln.sam ;done
...
for n in $(ls $Data/1_Raw_reads/*.humRef.unmapped.sam);do java -jar $Picard/picard.jar SamToFastq INPUT=$n FASTQ=${n%.*.*}.unmapped.1.fq SECOND_END_FASTQ=${n%.*.*}.unmapped.2.fq;done


####################################################################################
# 1.4. Remove low quality reads with Fastx toolkits
####################################################################################
for n in $(ls $Data/1_Raw_reads/*.humRef.unmapped.1.fq);do $Fastx/fastq_quality_filter -Q33  -q 20 -p 80 -i $n -o $Data/2_High_quality_reads/$(basename $n .humRef.unmapped.1.fq).clean.1.fq;done

for n in $(ls $Data/1_Raw_reads/*.humRef.unmapped.2.fq);do $Fastx/fastq_quality_filter -Q33  -q 20 -p 80 -i $n -o $Data/2_High_quality_reads/$(basename $n .humRef.unmapped.2.fq).clean.2.fq;done
####################################################################################
